...
public class CreatureHandler
{
	...
	*/
	public void updateCreatures() {
		...
		//wolf placeholder for when we spawn the new wolf
		Creature wolfToAdd=null;
		//plant placeholder for when we spawn the new plant
		Creature plantToAdd=null;
		for(Creature c : _creatures) {
				// Skip dead creatures
				...
				//spawn a new wolf to the left of the parent
				//or spawn a plant in place of a zombie
				if(c instanceof Spawner) {
					Spawner a = (Spawner)c;
					Creature newSpawn = a.spawnNewCreature();
					if (newSpawn instanceof Wolf ) {
						newSpawn.setLocation(new Point(c.getLocation().x,c.getLocation().y-1));
						wolfToAdd=newSpawn;
					}
					else if (newSpawn instanceof Plant) {
						newSpawn.setLocation(new Point(c.getLocation().x,c.getLocation().y-1));
						plantToAdd=newSpawn;
					}
				}
			}
		//here we add the wolf to the _creatures list
		//we cant add it in the loop because it throws a
		//Concurrent modification exception
		if (wolfToAdd!=null) {
			_creatures.add(wolfToAdd);
			System.out.println("new wolf spawned");
		}
		if (plantToAdd!=null) {
			_creatures.add(plantToAdd);
			System.out.println("new plant spawned");
		}
